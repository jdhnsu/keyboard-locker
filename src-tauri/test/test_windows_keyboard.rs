...
use windows_sys::Win32::UI::Input::{
	...
	HRAWINPUT, RAWINPUT, RAWINPUTDEVICE, RAWINPUTDEVICELIST, RAWINPUTHEADER, RID_INPUT,
	RIDI_DEVICENAME, RIDEV_EXINPUTSINK, RIM_TYPEKEYBOARD,
};
use windows_sys::Win32::UI::WindowsAndMessaging::{
	CreateWindowExW, DefWindowProcW, DispatchMessageW, GetMessageW, RegisterClassW,
	TranslateMessage, MSG, WM_INPUT, WNDCLASSW, HWND_MESSAGE,
};
...
fn read_raw_input(handle: HRAWINPUT) -> Option<RawKeyboardEvent> {
	unsafe {
		let mut size: u32 = 0;
...
fn enumerate_keyboard_devices() -> Vec<KeyboardDevice> {
	unsafe {
		let mut count: u32 = 0;
		...
		for device in devices.into_iter().take(count as usize) {
			...
			let name_query = GetRawInputDeviceInfoW(
				device.hDevice,
				RIDI_DEVICENAME,
				null_mut(),
				&mut name_len,
...
fn main() {
	println!("=== Windows Raw Input Keyboard Test ===");
	...
	if devices.is_empty() {
		...
		for (index, device) in devices.iter().enumerate() {
			println!(
				...
				index, device.handle, device.vendor_id, device.product_id, device.name
			);
		}
	}

	// Always create a message-only window — console windows cannot receive WM_INPUT reliably
	let target_hwnd = create_message_window();
	if target_hwnd.is_null() {
		eprintln!("failed to create message window: {}", unsafe { GetLastError() });
		return;
	}

	// RIDEV_EXINPUTSINK = receive input regardless of foreground status
	unsafe {
		...
		if RegisterRawInputDevices(&rid, 1, size_of::<RAWINPUTDEVICE>() as u32) == 0 {
			eprintln!("failed to register raw input: {}", GetLastError());
			return;
		}
	}

	println!("Raw Input registered. Press keys on different keyboards to compare device handles.");
